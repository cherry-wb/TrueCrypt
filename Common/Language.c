...
static char *MapNextLanguageFile ()
{
	...
	LanguageFileBuffer = (char *) malloc(find.nFileSizeLow);
...
BOOL LoadLanguageFile ()
{
	...
	for (res = (BYTE *) MapFirstLanguageFile (); res != NULL; res = (BYTE *) MapNextLanguageFile ())
	{
		xml = (char *) res;
			...
			while (xml = XmlFindElement (xml, xmlElements[i]))
			{
					...
					|| strcmp (attr, langId) == 0)
				{
					if (XmlGetAttributeText (xml, "key", attr, sizeof (attr)))
					{
						...
						// Parse \ escape sequences
						{
							...
							while (*in)
							{
								if (*in == '\\')
								{
									...
									default:
										MessageBoxA (0, (LPCSTR) key, "TrueCrypt: Unknown '\\' escape sequence in string", MB_ICONERROR);
										return FALSE;
									}
								}
								else
						...
						{
							MessageBoxA (0, (LPCSTR)key, "TrueCrypt: Error while decoding UTF-8 string", MB_ICONERROR);
							return FALSE;
						}
