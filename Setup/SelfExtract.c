...
static void __cdecl PipeWriteThread (void *len) 
{
	...
	{ 
		...
		if (!WriteFile ((HANDLE) hChildStdinWrite, (char *) PipeWriteBuf + bytesSentTotal, sendBufSize, (LPDWORD)&bytesSent, NULL) 
			|| bytesSent == 0
...
BOOL MakeSelfExtractingPackage (HWND hwndDlg, char *szDestDir)
{
	...
	GetModuleFileNameA (NULL, (LPSTR)inputFile, sizeof (inputFile));

	strcpy ((char *)outputFile, szDestDir);
	strncat ((char *)outputFile, OutputPackageFile, sizeof (outputFile) - strlen ((char *)outputFile) - 1);

	// Clone 'Setup.exe' to create the base of the new self-extracting archive

	if (!TCCopyFile ((char *)inputFile, (char *)outputFile))
	{
		handleWin32Error (hwndDlg);
		PkgError ("Cannot copy '" TC_SETUP_EXE "' to the package");
		goto err;
	}
	...
	{
		_snprintf ((char *)szTmpFilePath, sizeof(szTmpFilePath), "%s%s", szDestDir, szCompressedFiles[i]);

		if (!FileExists ((const char *)szTmpFilePath))
		{
			char tmpstr [1000];

			_snprintf (tmpstr, sizeof(tmpstr), "File not found:\n\n'%s'", szTmpFilePath);
			remove ((const char *)outputFile);
			PkgError (tmpstr);
			goto err;
		}

		bufLen += (int) GetFileSize64 ((const char *)szTmpFilePath);
	...
	if (!SaveBufferToFile (MAG_START_MARKER, (const char *)outputFile, strlen (MAG_START_MARKER), TRUE))
	{
	...
	for (i = 0; i < sizeof (szCompressedFiles) / sizeof (szCompressedFiles[0]); i++)
	{
		...
		tmpBuffer = (unsigned char *) LoadFile ((const char *)szTmpFilePath, &tmpFileSize);
	...
	if (!SaveBufferToFile ((const char *)szTmp32bit, (const char *)outputFile, sizeof (szTmp32bit), TRUE))
	{
		remove ((const char *)outputFile);
	...
	compressedBuffer = (unsigned char *) malloc (uncompressedDataLen + 524288);	// + 512K reserve
	...
	compressedDataLen = CompressBuffer ((char *)compressedBuffer, (char *)buffer, uncompressedDataLen);
	...
	if (!SaveBufferToFile ((const char *)compressedBuffer, (const char *)outputFile, compressedDataLen, TRUE))
	{
		remove ((const char *)outputFile);
	...
	if (!SaveBufferToFile ((const char *)MagEndMarker, (const char *)outputFile, strlen ((const char *)MagEndMarker), TRUE))
	{
		remove ((const char *)outputFile);
	...
	// Compute and write CRC-32 hash of the entire package
	{
		...
		tmpBuffer = (char *)LoadFile ((const char *)outputFile, &tmpFileSize);
...
BOOL VerifyPackageIntegrity (void)
{
	...
	fileDataEndPos = (int) FindStringInFile (path, (const char *)MagEndMarker, strlen ((const char *)MagEndMarker));
	if (fileDataEndPos < 0)
	{
	...
	if (!LoadInt32 (path, &crc, fileDataEndPos + strlen ((const char *)MagEndMarker) + 1))
	{
	...
	if (crc != GetCrc32 (tmpBuffer, fileDataEndPos + 1 + strlen ((const char *)MagEndMarker)))
	{
		free (tmpBuffer);
		Error ("DIST_PACKAGE_CORRUPTED");
		return FALSE;
...
BOOL IsSelfExtractingPackage (void)
{
	...
	return (FindStringInFile (path, (const char *)MagEndMarker, strlen ((const char *)MagEndMarker)) != -1);
}
...
BOOL SelfExtractInMemory (char *path)
{
	...
	DecompressedData = (unsigned char *) malloc (uncompressedLen + 524288);	// + 512K reserve 
	if (DecompressedData == NULL)
	{
		Error ("ERR_MEM_ALLOC");
		return FALSE;
	...
	// Decompress the data
	if ((unsigned int) DecompressBuffer ((char *)DecompressedData, (char *)compressedData, compressedLen) != uncompressedLen)
	{
		Error ("DIST_PACKAGE_CORRUPTED");
		goto sem_end;
	}
...
void __cdecl ExtractAllFilesThread (void *lpParam)
{
	int fileNo = 0;
	BOOL bSuccess = FALSE;
	char packageFile [TC_MAX_PATH] = { 0 };
	...
	for (fileNo = 0; fileNo < NBR_COMPRESSED_FILES; fileNo++)
	{
		...
		// Filename
		strncpy (fileName, (const char *)Decompressed_Files[fileNo].fileName, Decompressed_Files[fileNo].fileNameLength);
		...
		// Write the file
		if (!SaveBufferToFile (
			(const char *) Decompressed_Files[fileNo].fileContent,
			filePath,
