...
void __cdecl localcleanup (void)
{
	localcleanupwiz ();
	cleanup ();
...
BOOL DoFilesInstall (HWND hwndDlg, char *szDestDir)
{
	...
	for (i = 0; i < sizeof (szFiles) / sizeof (szFiles[0]); i++)
	{
		...
		if (bUninstall == FALSE)
		{
			...
			if (strstr (szFiles[i], "TrueCrypt Setup") != 0)
			{
				// Copy ourselves (the distribution package) to the destination location as 'Setup.exe'
...
BOOL DoShortcutsInstall (HWND hwndDlg, char *szDestDir, BOOL bProgGroup, BOOL bDesktopIcon)
{
	...
	if (bProgGroup)
	{
		...
		sprintf (szTmp, "%s%s", szDir, TC_SETUP_EXE);
		sprintf (szTmp2, "%s%s", szLinkDir, "\\Uninstall TrueCrypt.lnk");
		strcpy (szTmp3, "/u");
...
void __cdecl DoUninstall (void *arg)
{
...
void __cdecl DoInstall (void *arg)
{
	HWND hwndDlg = (HWND) arg;
	BOOL bOK = TRUE;
	char path[MAX_PATH];
	...
	strcat_s (path, sizeof (path), "\\" TC_SETUP_EXE);
	DeleteFile (path);
...
void SetInstallationPath (HWND hwndDlg)
{
	...
	if (RegOpenKeyEx (HKEY_LOCAL_MACHINE, "Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\TrueCrypt", 0, KEY_READ, &hkey) == ERROR_SUCCESS)
	{
		...
		------------------------------------------------------------------------------------
		1.0		C:\WINDOWS\Setup.exe /u			[optional]
		1.0a	C:\WINDOWS\Setup.exe /u			[optional]
		2.0		C:\WINDOWS\Setup.exe /u			[optional]
		2.1		C:\WINDOWS\Setup.exe /u			[optional]
		2.1a	C:\WINDOWS\Setup.exe /u			[optional]
		3.0		C:\WINDOWS\Setup.exe /u			[optional]
		3.0a	C:\WINDOWS\Setup.exe /u			[optional]
		3.1		The UninstallString was NEVER written (fortunately, 3.1a replaced 3.1 after 2 weeks)
		3.1a	C:\WINDOWS\Setup.exe /u
		4.0		C:\WINDOWS\Setup.exe /u C:\Program Files\TrueCrypt
		4.1		C:\WINDOWS\Setup.exe /u C:\Program Files\TrueCrypt
		4.2		C:\WINDOWS\Setup.exe /u C:\Program Files\TrueCrypt
		4.2a	C:\WINDOWS\Setup.exe /u C:\Program Files\TrueCrypt
		4.3		"C:\Program Files\TrueCrypt\Setup.exe" /u C:\Program Files\TrueCrypt\
		4.3a	"C:\Program Files\TrueCrypt\Setup.exe" /u C:\Program Files\TrueCrypt\
		5.0+	"C:\Program Files\TrueCrypt\Setup.exe" /u
